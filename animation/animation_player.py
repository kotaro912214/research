...
def update_animation(crnt_date_time, crnt_x, crnt_y, crnt_qly, total_sat):
    global pb_spd_prm
...
if (__name__ == '__main__'):
    ...
    ax_pos = plt.subplot(gs_anime[:, 0])
    # axes of X-Y Position
    ...
    # ax_hdop.grid()
    # axes of Time-Satellite Num
    ax_sat = plt.subplot(gs_anime[1, 1])
    ...
    while roop_count <= len_date_time-1:
        ...
        update_animation(crnt_date_time, crnt_x, crnt_y, crnt_qly, total_sat)
        if (pushed_start):
            roop_count += 1
